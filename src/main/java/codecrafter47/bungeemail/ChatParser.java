...
public class ChatParser {
    private static Pattern pattern = Pattern.compile("(?is)(?=\\n)|(?:[&\u00A7](?<color>[0-9A-FK-OR]))|" +
    ...
    private static Pattern strip_bbcode_pattern = Pattern.compile("(?is)(?:\\[(?<tag>/?(?:b|i|u|s|nocolor|nobbcode)|(?:url|command|hover|suggest|color)=(?<value>(?:(?:[^]\\[]*)\\[(?:[^]\\[]*)\\])*(?:[^]\\[]*))|/(?:url|command|hover|suggest|color))\\])|" +
            "(?:\\[(?<implicitTag>url|command|suggest)\\](?=(?<implicitValue>.*?)\\[/\\k<implicitTag>\\]))");
            ...
            if (group_tag != null && nobbcodeLevel <= 0) {
                ...
                if (group_tag.matches("(?is)^color=.*$")) {
                    ...
                    if (color != null && color != ChatColor.BOLD && color != ChatColor.ITALIC && color != ChatColor.MAGIC
                        ...
                        BungeeMail.instance.getLogger().warning("Invalid color tag: [" + group_tag + "] UNKNOWN COLOR '" + group_value + "'");
